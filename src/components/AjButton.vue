<template>
    <div class="space" :style="`
        ...
        `">
        <div class="transition-all duration-700 btn cursor-pointer relative h-14 bg-transparent hover:border-[1px] border-black hover:border-white" :class="{ 'border-[1px] border-white': active }">
            <div class="transition-all duration-700 opacity-0 relative top-0 left-0 z-20 bg-white w-4 h-full" :class="{ 'absolute top-0 left-0 opacity-100': active }"></div>
            <div class="transition-all duration-700 delay-300 absolute left-0 right-0 inset-y-0 z-10 flex items-center justify-center" :class="{ 'active': active }">
                <span class="text-gray-50 uppercase text-xl">{{ btnText }}</span>
            </div>
        </div>
    </div>
</template>
